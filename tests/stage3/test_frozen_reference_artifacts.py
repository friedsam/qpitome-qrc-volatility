...
RESULTS = ROOT / "results" / "modeling" / "transition_destination"


def test_frozen_path_bundle_is_internally_consistent() -> None:
    paths = np.load(RESULTS / "branch_destination_paths__paths.npy")
    metadata = pd.read_csv(RESULTS / "branch_destination_paths__episode_metadata.csv")
    standardization = pd.read_csv(RESULTS / "branch_destination_paths__standardization.csv")
...
def test_frozen_holdout_and_reference_metrics() -> None:
    metrics = pd.read_csv(
        RESULTS / "branch_destination_classical_baselines__metrics.csv"
    )
    ...
    rydberg = pd.read_csv(
        RESULTS / "branch_destination_rydberg_simulator__metrics.csv"
    ).set_index("model")
    assert np.isclose(
        rydberg.loc[
...
def test_outcome_maturity_is_respected_in_frozen_predictions() -> None:
    predictions = pd.read_csv(
        RESULTS / "branch_destination_classical_baselines__predictions.csv",
