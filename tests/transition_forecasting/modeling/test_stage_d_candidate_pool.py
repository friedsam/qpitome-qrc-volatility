...
from transition_forecasting.modeling.stage_d_candidate_pool import build_candidate_pool_from_series
...
def test_candidate_pool_uses_exact_trading_row_intervals_and_excludes_events() -> None:
    ...
    values = np.linspace(-4.0, -3.0, len(dates)) + 0.02 * np.sin(np.arange(len(dates)) / 5.0)
    ...
    assert not frame["origin_pos"].between(onset_position - 5, onset_position + 5).any()
