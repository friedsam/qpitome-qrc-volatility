...
DEFAULT_FALLBACK_DIR = Path("data/fallback/yahoo_daily_history")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download canonical raw SPY and VIX histories"
    )
    ...
    parser.add_argument(
        "--market-fallback",
        type=Path,
        default=DEFAULT_FALLBACK_DIR / "historical_market_data.csv",
    )
    parser.add_argument(
        "--volatility-fallback",
        type=Path,
        default=DEFAULT_FALLBACK_DIR / "historical_volatility_data.csv",
    )
    parser.add_argument(
        "--manifest-output",
        type=Path,
...
def main() -> None:
    ...
    result = download_market_data(
        ...
        market_fallback_path=args.market_fallback,
        volatility_fallback_path=args.volatility_fallback,
        manifest_path=args.manifest_output,
    )
    print(f"Market: {result.market_status}, rows={result.market_rows}")
    print(f"Volatility: {result.volatility_status}, rows={result.volatility_rows}")
