...
def monthly_realized_volatility(daily: pd.DataFrame, price_col: str, label: str) -> pd.DataFrame:
    ...
    rv = grouped.apply(lambda series: float(np.sqrt(np.square(series.dropna()).sum())))
    out = pd.DataFrame(
        {
    ...
    out[f"log_rv_{label}"] = np.log(out[f"rv_{label}"].where(out[f"rv_{label}"] > 0))
    out[f"rv_q3_mean_{label}"] = out[f"rv_{label}"].rolling(3, min_periods=3).mean()
    out[f"rv_a12_mean_{label}"] = out[f"rv_{label}"].rolling(12, min_periods=12).mean()
    ...
    out[f"log_rv_q3_mean_{label}"] = out[f"log_rv_{label}"].rolling(3, min_periods=3).mean()
    out[f"log_rv_a12_mean_{label}"] = out[f"log_rv_{label}"].rolling(12, min_periods=12).mean()
    return out
...
def add_feature(catalog: list[dict], frame: pd.DataFrame, name: str, values: pd.Series,
                family: str, role: str, formula: str,
                timing: str = "available by end of month t") -> None:
    frame[name] = values
    catalog.append(
        {
            "feature": name,
...
def main() -> None:
    ...
    if adjusted_col:
        monthly = merge(monthly, monthly_realized_volatility(daily, adjusted_col, "adjclose"))
    ...
    monthly = merge(monthly, read_french_zip(factors / "ff3_monthly.zip", ["mkt_excess", "smb", "hml", "rf"]))
    monthly = merge(monthly, read_french_zip(factors / "short_term_reversal_monthly.zip", ["str"]))
    ...
    monthly["inflation_log_change__avail_lag1"] = monthly["inflation_log_change"].shift(1)
    ...
    current_month = pd.Timestamp(datetime.now(timezone.utc).date()).to_period("M").to_timestamp()
    extended = monthly.loc[monthly["date"] < current_month].copy()
    paper = monthly.loc[(monthly["date"] >= PAPER_START) & (monthly["date"] <= PAPER_END)].copy()
    if len(paper) != PAPER_ROWS:
        raise ValueError(f"Paper slice has {len(paper)} rows; expected {PAPER_ROWS}")
    ...
    catalog: list[dict] = []
    add_feature(catalog, features, "vol_state_1m", y, "volatility_state", "current state", "log_rv_close")
    add_feature(catalog, features, "vol_state_3m_mean", y.rolling(3, min_periods=3).mean(), "volatility_state", "short memory", "mean(log_rv[t-2:t])")
    add_feature(catalog, features, "vol_state_12m_mean", y.rolling(12, min_periods=12).mean(), "volatility_state", "long memory", "mean(log_rv[t-11:t])")
    delta = y.diff()
    add_feature(catalog, features, "vol_change_1m", delta, "volatility_dynamics", "rate of change", "log_rv[t]-log_rv[t-1]")
    add_feature(catalog, features, "vol_change_3m", y - y.shift(3), "volatility_dynamics", "medium change", "log_rv[t]-log_rv[t-3]")
    add_feature(catalog, features, "vol_acceleration_1m", delta.diff(), "volatility_dynamics", "acceleration", "delta1[t]-delta1[t-1]")
    add_feature(catalog, features, "vol_short_minus_long", features["vol_state_3m_mean"] - features["vol_state_12m_mean"], "volatility_dynamics", "timescale separation", "mean3-mean12")
    add_feature(catalog, features, "vol_prior_12m_robust_z", robust_z(y), "volatility_dynamics", "robust surprise", "prior-12-month median/MAD")

    for column in ["mkt_excess", "smb", "hml", "str"]:
        add_feature(catalog, features, f"market_{column}", paper[column], "market_drivers", "market shock", column)
    for column in ["tb3ms", "aaa_yield", "baa_yield", "default_spread_baa_minus_aaa__candidate"]:
        base = column.replace("__candidate", "")
        add_feature(catalog, features, f"credit_{base}_level", paper[column], "credit_rates", "state", column)
        add_feature(catalog, features, f"credit_{base}_change_1m", paper[column].diff(), "credit_rates", "change", f"{column}[t]-{column}[t-1]")
    for source, name in {
        "inflation_log_change__avail_lag1": "macro_inflation_growth_lag1",
        "ip_log_growth__avail_lag1": "macro_ip_growth_lag1",
    }.items():
        add_feature(catalog, features, name, paper[source], "macro_dynamics", "lagged growth", source, "one-month availability lag enforced")
        add_feature(catalog, features, f"{name}_change_1m", paper[source].diff(), "macro_dynamics", "acceleration", f"{source}[t]-{source}[t-1]", "one-month availability lag enforced")

    features["target_log_rv_t_plus_1"] = y.shift(-1)
    features["target_log_rv_t_plus_5"] = y.shift(-5)
    feature_dir = OUT / "features"
    feature_dir.mkdir(parents=True, exist_ok=True)
    features.to_parquet(feature_dir / "preliminary_features.parquet", index=False)
    pd.DataFrame(catalog).to_csv(feature_dir / "feature_catalog.csv", index=False)
    ...
    (feature_dir / "feature_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
