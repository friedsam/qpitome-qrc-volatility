...
from transition_forecasting.qrc.bivariate_carrier_crossmix_assay import (
    ...
    run_bivariate_carrier_crossmix_assay,
)
...
DEFAULT_RESULTS_ROOT = Path(
    "results/transition_forecasting/qrc/run_bivariate_carrier_crossmix_assay"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare the 0.015 us pure-slot all-orders baseline with carrier-assisted "
            "identity and Hadamard cross-mixed global input masks."
        )
    )
...
def main() -> None:
    ...
    run_dir = run_bivariate_carrier_crossmix_assay(
