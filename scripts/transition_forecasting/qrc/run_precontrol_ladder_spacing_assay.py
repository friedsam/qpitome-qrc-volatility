...
from transition_forecasting.qrc.precontrol_ladder_spacing_assay import (
    ...
    run_precontrol_ladder_spacing_assay,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare only 8, 9, and 10 um ladder row spacing on the frozen "
            "pre-control segmented-readout system."
        )
    )
    ...
    parser.add_argument(
        "--later-folds", type=int, nargs="+", default=[4, 5, 6, 7, 8]
    )
    ...
    parser.add_argument("--incumbent-row-spacing-um", type=float, default=9.0)
    parser.add_argument("--incumbent-interaction-scale", type=float, default=1.25)
...
def main() -> None:
    ...
    spacing_config = PrecontrolSpacingAssayConfig(
        ...
        row_spacings_um=tuple(args.row_spacings_um),
        incumbent_row_spacing_um=args.incumbent_row_spacing_um,
        incumbent_interaction_scale=args.incumbent_interaction_scale,
    ...
    readout_config = LadderReadoutUpgradeConfig(
        ...
        ladder_interaction_scale=args.incumbent_interaction_scale,
        seed=args.selection_seed,
    )
    ...
    incumbent_geometry = StaggeredLadderGeometryConfig(
        ...
        row_spacing_um=args.incumbent_row_spacing_um,
    ...
    run_dir = run_precontrol_ladder_spacing_assay(
        ...
        reservoir=reservoir,
        incumbent_geometry=incumbent_geometry,
        run_id=args.run_id,
    )
