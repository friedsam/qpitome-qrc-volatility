...
DEFAULT_RESULTS_ROOT = Path(
    "results/transition_forecasting/qrc/run_bivariate_capacity_assay"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Measure bivariate memory, nonlinear mixing, and order capacity of the "
            "frozen six-atom ladder while separating encoding and readout compression."
        )
    )
    ...
    parser.add_argument("--sequential-first-slot-fraction", type=float, default=0.50)
    parser.add_argument(
        "--sequential-second-phase-rad", type=float, default=1.5707963267948966
    )
...
def main() -> None:
    ...
    config = BivariateCapacityConfig(
        ...
        sequential_first_slot_fraction=args.sequential_first_slot_fraction,
        sequential_second_phase_rad=args.sequential_second_phase_rad,
        seed=args.seed,
    )
    ...
    run_dir = run_bivariate_capacity_assay(
