...
from transition_forecasting.qrc.bivariate_crossover_duration_assay import (
    ...
    run_bivariate_crossover_duration_assay,
)
...
DEFAULT_RESULTS_ROOT = Path(
    "results/transition_forecasting/qrc/run_bivariate_crossover_duration_assay"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep the input duration of the exchange-paired palindromic crossover and "
            "measure balanced memory, delay-5 capacity, and nonlinear mixing."
        )
    )
...
def main() -> None:
    args = parse_args()
    config = BivariateCrossoverDurationConfig(
        samples=args.samples,
        sequence_length=args.sequence_length,
        memory_delays=tuple(args.memory_delays),
        alphas=tuple(args.alphas),
        seeds=tuple(args.seeds),
        step_durations_us=tuple(args.step_durations_us),
    ...
    run_dir = run_bivariate_crossover_duration_assay(
