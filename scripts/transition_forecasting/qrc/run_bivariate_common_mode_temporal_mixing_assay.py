...
from transition_forecasting.qrc.bivariate_common_mode_temporal_mixing_assay import (
    ...
    run_bivariate_common_mode_temporal_mixing_assay,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Duplicate one scalar stream across both nominal channels and compare "
            "single-route, simultaneous, ordered, and palindromic global-control "
            "encodings on nonlinear temporal-memory targets."
        )
    )
...
def main() -> None:
    ...
    run_dir = run_bivariate_common_mode_temporal_mixing_assay(
