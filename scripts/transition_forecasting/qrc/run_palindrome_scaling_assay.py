...
DEFAULT_RESULTS_ROOT = Path(
    "results/transition_forecasting/qrc/palindrome_scaling_assay"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Measure exact final-palindrome performance scaling on a bounded atom grid "
            "and report statevector resources through 20 atoms."
        )
    )
    ...
    parser.add_argument("--resource-min-atoms", type=int, default=5)
    parser.add_argument("--resource-max-atoms", type=int, default=20)
    parser.add_argument("--runtime-fit-tail", type=int, default=4)
    return parser.parse_args()


def frozen_reservoir_template() -> TemporalRydbergChainConfig:
...
def main() -> None:
    ...
    assay = PalindromeScalingConfig(
        ...
        exact_atom_counts=tuple(args.exact_atoms),
        resource_min_atoms=args.resource_min_atoms,
        resource_max_atoms=args.resource_max_atoms,
        runtime_fit_tail=args.runtime_fit_tail,
    )
    run_dir = run_palindrome_scaling_assay(
        fold_dir=args.fold_dir,
        results_root=args.out_root,
        assay=assay,
        candidate_features=CandidateFeatureConfig(),
        reservoir_template=frozen_reservoir_template(),
