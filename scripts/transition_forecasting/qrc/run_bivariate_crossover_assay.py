...
DEFAULT_RESULTS_ROOT = Path(
    "results/transition_forecasting/qrc/run_bivariate_crossover_assay"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Test symmetric amplitude/detuning crossover schedules for balanced "
            "bivariate memory and permutation-controlled nonlinear mixing."
        )
    )
...
def main() -> None:
    ...
    config = BivariateCrossoverConfig(
        ...
        feature_banks=tuple(args.feature_banks),
    )
    ...
    run_dir = run_bivariate_crossover_assay(
