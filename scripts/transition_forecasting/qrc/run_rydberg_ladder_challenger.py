...
from transition_forecasting.qrc.rydberg_ladder_challenger import (
    run_rydberg_ladder_challenger,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare a calibrated noninteracting chain, interacting chain, "
            "and six-atom staggered Rydberg ladder using the frozen "
            "level-plus-instability readout."
        )
    )
    ...
    parser.add_argument(
        "--ladder-interaction-scales",
        type=float,
        nargs="+",
        default=[0.75, 1.0, 1.25],
    )
    parser.add_argument("--control-interaction-scale", type=float, default=1.0)
    parser.add_argument(
        "--controls",
        nargs="+",
        choices=["reset", "shuffled", "reversed", "block_shuffled"],
...
def main() -> None:
    ...
    challenger = LadderChallengerConfig(
        ...
        ladder_interaction_scales=tuple(args.ladder_interaction_scales),
        control_interaction_scale=args.control_interaction_scale,
    ...
    run_dir = run_rydberg_ladder_challenger(
