...
from transition_forecasting.qrc.bivariate_encoding_symmetry_assay import (
    BivariateEncodingSymmetryConfig,
    run_bivariate_encoding_symmetry_assay,
)
...
DEFAULT_RESULTS_ROOT = Path(
    "results/transition_forecasting/qrc/run_bivariate_encoding_symmetry_assay"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Use paired exchangeable input streams to determine whether bivariate "
            "memory imbalance follows amplitude/detuning routing or slot recency."
        )
    )
...
def main() -> None:
    args = parse_args()
    config = BivariateEncodingSymmetryConfig(
    ...
    run_dir = run_bivariate_encoding_symmetry_assay(
