...
def har_features(sequence: np.ndarray) -> np.ndarray:
    level = sequence[:, -1]
    return np.column_stack([level, sequence[:, -5:].mean(axis=1), sequence[:, -20:].mean(axis=1)])
    ...
    if "lead" in frame.columns:
        ...
        for lead in sorted(pd.Series(leads).dropna().unique()):
            ...
            if "label" in frame.columns:
                ...
                groups.extend(
                    [
                        ("lead_label", f"L{int(lead)}_transition", lead_mask & (labels == 1)),
                        ("lead_label", f"L{int(lead)}_control", lead_mask & (labels == 0)),
                    ]
                )
            ...
            rows.append(
                {
                    ...
                    "qlike": float(qlike(yt[:, horizon], yp[:, horizon]).mean()),
                }
            )
    return rows
...
def run_frozen_fold_baselines(
    ...
    *,
    ...
    return metrics, predictions, {"fold_counts": fold_counts, "aggregate_path_metrics": aggregate}
...
def build_broad_samples(panel_path: Path, *, stride: int) -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    raw = raw[(raw["high"] > 0) & (raw["low"] > 0) & (raw["high"] >= raw["low"])].copy()
    ...
    for ticker, group in raw.groupby("ticker", sort=True):
        ...
        for origin in range(WINDOW - 1, len(series) - HORIZON, stride):
            ...
            if not np.isfinite(sequence).all() or not np.isfinite(target).all():
                continue
            date = dates[origin]
            split = "train" if date < VAL_START else "val" if date < TEST_START else "test"
            rows.append(
                {
...
def compare_with_reference(metrics: pd.DataFrame, reference_path: Path) -> dict[str, object]:
    ...
    for model, expected_metrics in expected["path_metrics"].items():
        row = path.loc[model]
        for metric, tolerance_key in [("rmse", "rmse_absolute"), ("qlike", "qlike_absolute")]:
        ...
        [
            {
                ...
                "passed": float(path.loc["har", "rmse"]) < float(path.loc["persistence", "rmse"]),
            },
            {
                "check": "field_raw_ridge_beats_har_rmse",
                "passed": float(path.loc["raw_sequence_ridge", "rmse"]) < float(path.loc["har", "rmse"]),
            },
        ]
    )
    return {
...
def frozen_sanity(metrics: pd.DataFrame) -> dict[str, object]:
    ...
    def value(model: str, group_type: str, group_value: str, metric: str) -> float:
        ...
        if len(match) != 1:
            raise ValueError(f"missing unique aggregate metric for {model}/{group_type}/{group_value}")
    ...
    checks = [
        ...
        },
        {
            ...
            "passed": value("raw_sequence_ridge", "label", "transition", "mean_qlike")
            < value("raw_sequence_ridge_shuffled", "label", "transition", "mean_qlike"),
        },
    ]
    return {
        "checks": checks,
        "all_checks_passed": all(bool(check["passed"]) for check in checks),
        "interpretation": "Failure is a scientific result unless caused by an implementation or data-integrity error.",
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run primitive classical baselines on frozen folds and reproduce the field experiment."
    )
    ...
    parser.add_argument(
        "--reference",
        type=Path,
        default=Path("config/transition_forecasting/field_experiment_reference.json"),
    )
...
def main() -> int:
    ...
    predictions = pd.concat([fold_predictions, field_predictions], ignore_index=True, sort=False)
    ...
    return 0 if field_comparison["all_checks_passed"] else 1
