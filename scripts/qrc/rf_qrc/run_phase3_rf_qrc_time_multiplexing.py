...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    results_dir = args.results_dir or (root / "results" / "qrc" / "rf_qrc")
    ...
    max_nodes = max(args.virtual_nodes)
    print(f"Building RF-QRC ring feature matrix with max_virtual_nodes={max_nodes}")
    fmap = TimeMultiplexedRFQRCMap(args.n_qubits, args.input_scale, args.random_scale, args.seed)
    Fmax = fmap.transform(U, max_virtual_nodes=max_nodes, layer_scale=args.layer_scale)
    per_node_dim = args.n_qubits + args.n_qubits * (args.n_qubits - 1) // 2

    metrics_all = []
    preds = pd.DataFrame({"date": dates, "actual_future_rv_20d": y, "split": split})

    for nodes in args.virtual_nodes:
        F = Fmax[:, : nodes * per_node_dim]
        for alpha in args.ridge_alphas:
            run_name = f"rf_qrc_ring_tm_vn{nodes}_alpha{int(alpha)}"
            print(f"Running {run_name}")
            metrics, pred = fit_eval(F, y, split, alpha)
            metrics.insert(0, "run_name", run_name)
            metrics.insert(1, "virtual_nodes", nodes)
            metrics.insert(2, "ridge_alpha", alpha)
            metrics_all.append(metrics)
            preds[f"{run_name}_pred"] = pred

    metrics_df = pd.concat(metrics_all, ignore_index=True)
    metrics_path = results_dir / "phase3_rf_qrc_time_multiplex_metrics.csv"
    pred_path = results_dir / "phase3_rf_qrc_time_multiplex_predictions.csv"
    summary_path = results_dir / "phase3_rf_qrc_time_multiplex_test_summary.csv"
    ...
    show_cols = [
        "run_name", "virtual_nodes", "ridge_alpha", "rmse", "qlike", "corr", "actual_std", "pred_std",
        "q80_f1", "q90_f1", "q95_f1", "top20_pred_actual_ratio", "effective_rank",
    ]
    print("\nTest summary:")
    print(metrics_df.loc[metrics_df["split"].eq("test"), show_cols].sort_values(["q95_f1", "qlike"], ascending=[False, True]).to_string(index=False))
    return 0
