...
def make_folds(n: int, *, n_folds: int, min_train: int, val_size: int, purge: int) -> list[dict]:
    ...
    The shared protocol records the explicit purge interval. Historically,
    run_master_comparison.make_folds returned only fold/train/val/test, so this
    wrapper preserves that exact public shape for existing audit scripts.
    """
