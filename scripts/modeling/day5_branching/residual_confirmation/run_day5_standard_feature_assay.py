...
from qpitome_qrc.day5.protocol import eligible_rows, load_frame
...
def prepare_features(frame: pd.DataFrame, preparation: str | None) -> pd.DataFrame:
    ...
    if preparation == "path_shape":
        return add_path_shape_features(frame)
...
def historical_crossfit_logits(train: pd.DataFrame, baseline: list[str]) -> tuple[np.ndarray, np.ndarray]:
    """Return historical cross-fitted logits for an arbitrary declared baseline."""
...
def score_deltas(frame: pd.DataFrame, baseline_col: str, model_col: str) -> dict[str, float | int | str]:
    """Return the historical assay delta schema via shared proper scoring."""
...
def run(frame: pd.DataFrame, spec: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for fold_number, (cluster_start, test_group) in enumerate(fold_groups, start=1):
        ...
        for block_pos, (block_name, block_spec) in enumerate(blocks.items()):
            ...
            baseline_model = logistic_pipeline(1.0)
            ...
            raw_model = logistic_pipeline(1.0)
            raw_model.fit(np.column_stack([B_train, H_train]), y_train)
            fold_outputs[f"raw__{block_name}"] = raw_model.predict_proba(
                np.column_stack([B_test, H_test])
            )[:, 1]
            ...
            X_train = d1_basis(B_train, "quadratic")
            X_test = d1_basis(B_test, "quadratic")
            ...
            offset_test = logit(p_base)

            maps = {
                "linear": (R_train, R_test),
                **nonlinear_maps(R_train, R_test, RNG_SEED + 1000 * block_pos + fold_number),
            }
            for map_pos, (map_name, (M_train, M_test)) in enumerate(maps.items()):
                ...
                rng = np.random.default_rng(
                    RNG_SEED + 100000 + 1000 * block_pos + 10 * map_pos + fold_number
                )
    ...
    return (
        pd.DataFrame(predictions).sort_values("row_id").reset_index(drop=True),
...
def main() -> None:
    ...
    parser.add_argument("--outdir", type=Path, default=Path("results/modeling/day5_branching/residual_confirmation/day5_standard_feature_assay"))
