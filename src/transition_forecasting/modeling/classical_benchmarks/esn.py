...
DEFAULT_ALPHAS = (10.0, 30.0, 100.0, 300.0, 1000.0, 3000.0, 10000.0)
DEFAULT_SEEDS = (1, 2, 3)
DEFAULT_CONFIGS = (
    {"config_id": "short_inp020", "n": 300, "connectivity": 0.02, "sr": 0.55, "inp": 0.20, "leak": 0.80},
    {"config_id": "strong_inp090", "n": 300, "connectivity": 0.02, "sr": 0.90, "inp": 0.90, "leak": 0.30},
    {"config_id": "compact_conn001", "n": 150, "connectivity": 0.01, "sr": 0.90, "inp": 0.30, "leak": 0.30},
)
...
def pooled_features_vectorized(
    ...
    *,
    ...
    """Equivalent final/mean/std ESN pooling, batched across independent windows."""
    ...
    return np.concatenate([state, mean, std], axis=1)


def _seed_features(inputs: np.ndarray, config: dict[str, object], seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    ordered = pooled_features_vectorized(inputs, w_in=w_in, w=w, leak=float(config["leak"]))
    shuffled = pooled_features_vectorized(_shuffle_windows(inputs, seed), w_in=w_in, w=w, leak=float(config["leak"]))
...
def _ridge_path_predictions(
    ...
    """Exact standardized Ridge(alpha, fit_intercept=True) predictions from one SVD."""
    ...
    return {
        float(alpha): target_mean + (val_projection * (singular / (singular * singular + float(alpha)))) @ target_projection
        for alpha in alphas
    }


def _readout_prediction(features: np.ndarray, target: np.ndarray, train_mask: np.ndarray, val_mask: np.ndarray, alpha: float) -> np.ndarray:
    return _ridge_path_predictions(features, target, train_mask, val_mask, (float(alpha),))[float(alpha)]


def _evaluate_config_fold(dataset, fold: int, config: dict[str, object], seeds: tuple[int, ...], alphas: tuple[float, ...]):
    ...
    predictions: dict[tuple[str, float], list[np.ndarray]] = {
        (order, float(alpha)): [] for order in ("ordered", "shuffled") for alpha in alphas
    }
    seed_rows = []
    for seed in seeds:
        ordered, shuffled = _seed_features(inputs, config, seed)
        for order, features in (("ordered", ordered), ("shuffled", shuffled)):
            path = _ridge_path_predictions(features, target, train_mask, val_mask, alphas)
            for alpha, prediction in path.items():
                predictions[(order, float(alpha))].append(prediction)
                seed_rows.append({
                    "config_id": config["config_id"], "fold": fold, "order": order, "seed": seed, "alpha": alpha,
                    "qlike": float(qlike_loss(y_val, prediction).mean()),
                    "rmse": float(np.sqrt(np.mean((y_val - prediction) ** 2))),
                })
    tuning_rows = []
    for (order, alpha), seed_predictions in predictions.items():
        ensemble = np.mean(seed_predictions, axis=0)
        tuning_rows.append({
            "config_id": config["config_id"], "fold": fold, "order": order, "alpha": alpha,
            "ensemble_qlike": float(qlike_loss(y_val, ensemble).mean()),
            "ensemble_rmse": float(np.sqrt(np.mean((y_val - ensemble) ** 2))),
            "n_samples": int(val_mask.sum()),
        })
    return pd.DataFrame(tuning_rows), pd.DataFrame(seed_rows)


def select_esn_spec(dataset, *, folds, configs, seeds, alphas):
    tuning_frames = []
    seed_frames = []
    for config in configs:
        for fold in folds:
            print(f"ESN selection config={config['config_id']} fold={fold}", flush=True)
            tuning, seed_metrics = _evaluate_config_fold(dataset, fold, config, seeds, alphas)
    ...
    ordered = tuning[tuning["order"].eq("ordered")]
    summary = (
        ordered.groupby(["config_id", "alpha"], as_index=False)
        .agg(
            mean_fold_qlike=("ensemble_qlike", "mean"),
            std_fold_qlike=("ensemble_qlike", "std"),
            mean_fold_rmse=("ensemble_rmse", "mean"),
        )
        .sort_values(["mean_fold_qlike", "mean_fold_rmse", "config_id", "alpha"], ignore_index=True)
    )
    winner = summary.iloc[0]
    config = next(dict(item) for item in configs if item["config_id"] == winner["config_id"])
    return config, float(winner["alpha"]), tuning, seed_metrics, summary


def predict_fold(dataset, *, fold: int, config: dict[str, object], alpha: float, seeds: tuple[int, ...]) -> pd.DataFrame:
    frame, sequences, train_mask, val_mask = _fold_rows(dataset, fold)
    target = frame[list(TARGET_COLUMNS)].to_numpy(dtype=float)
    y_val = target[val_mask]
    inputs = _scale_channels(transform_input(sequences, REPRESENTATION), train_mask)
    ...
    for seed in seeds:
        ordered, shuffled = _seed_features(inputs, config, seed)
        ordered_predictions.append(_readout_prediction(ordered, target, train_mask, val_mask, alpha))
        shuffled_predictions.append(_readout_prediction(shuffled, target, train_mask, val_mask, alpha))
    predictions = {
        "esn_direct": np.mean(ordered_predictions, axis=0),
        "esn_shuffled": np.mean(shuffled_predictions, axis=0),
    }
