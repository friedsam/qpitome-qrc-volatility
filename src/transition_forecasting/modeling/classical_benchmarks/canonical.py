...
def _load_predictions(run_dir: Path, models: tuple[str, ...]) -> pd.DataFrame:
    frame = pd.read_csv(Path(run_dir) / "predictions.csv.gz")
    frame = frame[frame["model"].isin(models)].copy()
    finite = np.all(
        np.isfinite(frame[PREDICTED_COLUMNS].to_numpy(dtype=float)),
        axis=1,
    )
...
def _validate_actuals(predictions: pd.DataFrame) -> None:
    ...
    for model in MODEL_ORDER:
        ...
        if not np.allclose(
            current[ACTUAL_COLUMNS].to_numpy(dtype=float),
            reference[ACTUAL_COLUMNS].to_numpy(dtype=float),
        ):
...
def _stage_metrics(
    ...
    return path, horizon, grouped_fold_metrics(predictions)


def _paired_deltas(
    metrics: pd.DataFrame,
    reference_model: str = "sequence_ridge",
) -> pd.DataFrame:
...
def run_canonical_comparison(
    *,
    ...
    esn = _load_predictions(esn_run, ("esn_direct_tuned", "esn_shuffled_tuned"))
    all_predictions = pd.concat([linear, garch, esn], ignore_index=True, sort=False)
    keys_by_model = {
        model: _available_keys(all_predictions, model)
        for model in MODEL_ORDER
    }
    ...
    order = pd.Categorical(predictions["model"], categories=MODEL_ORDER, ordered=True)
    predictions = (
        predictions.assign(_model_order=order)
        .sort_values(["fold", "sample_id", "_model_order"])
        .drop(columns="_model_order")
    )
    metrics, metrics_by_horizon, metrics_by_fold = _stage_metrics(
        predictions,
        selection_folds,
        confirmation_folds,
    )
    ...
    union_keys = set.union(*(keys_by_model[model] for model in MODEL_ORDER))
    coverage = pd.DataFrame(
        [
            {
                "model": model,
                "available_rows": len(keys_by_model[model]),
                "common_rows": len(common_keys),
                "union_rows": len(union_keys),
                "available_fraction_of_union": len(keys_by_model[model]) / len(union_keys),
                "common_fraction_of_available": len(common_keys) / len(keys_by_model[model]),
            }
            for model in MODEL_ORDER
        ]
    )
    ...
    for stage in ("selection", "confirmation", "development_all"):
        metrics[metrics["stage"].eq(stage)].to_csv(
            run_dir / f"submission_table_{stage}.csv",
            index=False,
        )
    summary = {
        "status": "complete",
