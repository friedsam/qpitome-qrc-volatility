...
def _load_close_panel(path: Path) -> dict[str, pd.Series]:
    ...
    frame = frame.sort_values(["index", "date"])
    return {
        str(name): group.set_index("date")["close"].astype(float)
        for name, group in frame.groupby("index", sort=False)
    }
...
def _return_history(
    ...
    returns = np.log(close.loc[:origin]).diff().dropna()
    if history > 0:
...
def _fit_one(task: tuple[int, str, str, int, int, dict[str, object]]) -> dict[str, object]:
    ...
    prediction = variance_path_to_log_volatility_path(
...
def run_garch_benchmark(
    *,
    ...
    source = dataset.manifest[
        dataset.manifest["fold"].isin(all_folds)
        & dataset.manifest["fold_split"].eq("val")
    ...
    grouped_fold_metrics(scored).to_csv(run_dir / "metrics_by_fold.csv", index=False)
    horizon_metrics.to_csv(run_dir / "metrics_by_horizon.csv", index=False)
    output[
        [
            "sample_id", "fold", "index", "origin_date", "history_rows",
            "converged", "fit_note", "parameters_json",
        ]
    ].to_csv(run_dir / "fit_diagnostics.csv.gz", index=False, compression="gzip")
    ...
    (run_dir / "runtime.json").write_text(
        json.dumps(
            {"wall_seconds": time.perf_counter() - started, "max_workers": workers},
            indent=2,
        ) + "\n",
        encoding="utf-8",
    )
