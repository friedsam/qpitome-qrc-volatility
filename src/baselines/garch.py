...
def _fit_with_scipy(values: np.ndarray, horizon: int, cfg: GARCHConfig) -> GARCHForecast:
    ...
    start = _initial_theta(variance, 0.05, 0.90, 8.0)
    result = minimize(
        _student_t_nll,
        start,
        args=(scaled,),
    ...
    omega, alpha, beta, nu = _decode(result.x)
    ...
    return GARCHForecast(
        ...
        converged=converged,
        convergence_flag=int(result.status),
        parameters=parameters,
        note=("converged:scipy" if converged else f"nonconvergence:scipy:{result.status}:{result.message}"),
    )
