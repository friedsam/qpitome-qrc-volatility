"""Structured level/rate RF-QRC variant from the Phase 3 ablation study.

The implementation preserves the original experiment numerically while reusing
low-level RF-QRC statevector primitives from ``rf_qrc_reservoir``.
"""
...
from qpitome_qrc.qrc.rf_qrc_reservoir import (
    apply_cnot,
    apply_one,
    apply_zz_phase,
    ry,
...
def structured_entangler_pairs(
    mode: StructuredEntangler,
    n_qubits: int,
) -> list[tuple[int, int]]:
    """Return the exact ordered CNOT pairs used by the legacy ablation."""

    if n_qubits % 2 != 0:
        raise ValueError("Structured level-rate RF-QRC requires an even number of qubits")

    k = n_qubits // 2
    level = list(range(k))
    rate = list(range(k, n_qubits))
...
class StructuredLevelRateRFQRCMap:
    """RF-QRC map with separate level/rate blocks and structured couplings."""

    def __init__(
        self,
        n_qubits: int,
        entangler: StructuredEntangler,
        input_scale: float,
        level_scale: float,
        rate_scale: float,
        random_scale: float,
    ...
    def _entangle(self, state: np.ndarray) -> np.ndarray:
        for i, j in structured_entangler_pairs(self.entangler, self.n):
        ...
        for i in range(self.n):
            for j in range(i + 1, self.n):
                ...
                state = apply_zz_phase(
                    ...
                    i,
                    j,
                    float(boost * self.zz_angles[i, j]),
                    self.n,
                )
        return state
    ...
    def one(self, u: np.ndarray) -> np.ndarray:
        ...
        if values.shape != (self.n,):
            raise ValueError(f"Expected input shape ({self.n},), got {values.shape}")

        state = np.zeros(2**self.n, dtype=complex)
        state[0] = 1.0
        state = self._encode(state, values, second=False)
        state = self._entangle(state)
        state = self._encode(state, values, second=True)
