...
def credential_status() -> dict[str, object]:
    ...
    return {
        ...
        "configured": any(bool(item["exists"]) for item in records),
        "secure": any(bool(item["secure_mode"]) for item in records),
    }
...
def build_report() -> dict[str, object]:
    ...
    if not live_ready and not fallback_ready:
        warnings.append(
            "No usable transition-data source is available: configure secure Kaggle credentials or provide the verified fallback"
        )
